...
:- [agent_helpers].
agent_name(greedy_agent1).

init_agent(greedy_agent1):-
	asserta(agent_state(init)).

%Init
move(greedy_agent1,_,_):-
	agent_state(init),
	set_agent_state(decide_state),
	fail.

%Go home - can't reach fuel station
move(greedy_agent1, MoveType, MoveQuantity):-
	needs_fuel,
	moves_remaining(MRem),
	player(greedy_agent1, WL, Cash, VL, Fuel, Where),
	findall(
